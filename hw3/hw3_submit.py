...
class ShallowCNN:
    ...
    def backpropagation(self, learning_rate):  
        back = self.lastlayer.backward()
        for layer in reversed(self.layers.values()):
            back = layer.backward(back)
        
        for layer in self.layers.keys():
            if layer[0] == 'C' or layer[0] =='F':
                self.layers[layer].W -= learning_rate * self.layers[layer].dW
                self.layers[layer].b -= learning_rate * self.layers[layer].db        
                #print(i,layer)
...
ConvLayerlist =  [['C',20, 5, 3, 2, 2],
    ['C', 20, 3, 20, 1, 1],
    ['P', 2, 2, 0],
    ['C', 20, 1, 20, 1, 0],
    ['P', 2, 2, 0]]
FullLayerlist = [[4*4*20, 50],
     [50, 10]]
